...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id" :class="{ highlight: highlightingEnabled && author.birthYear > 1850 }">
      ...
      <ul>
        <li v-for="work in austen?.famousWorks" :key="work.title">
          {{ work.title }} (Published: {{ work.year }})
        </li>
      </ul>
    </section>
  ...
        <p>Highlighting Specific Authors:</p>
        <p>Click the button below to highlight authors born after 1850 in the first list.</p>
        <button @click="highlightingEnabled = !highlightingEnabled">
          Toggle Modern Author Highlight
        </button>
      </section>
  </div>
</template>
...
const highlightingEnabled = ref(false)
